...
`packet-model-metric-grid` is centered on ml utilities work in Haskell. The technical target is: Create a Haskell reference implementation for metric workflows, centered on constraint solving, bounded scenario files, and conflict explanations.
...
The score is deterministic. The current threshold is 183. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 6.
